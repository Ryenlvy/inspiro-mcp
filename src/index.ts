...
const API_KEY = process.env.INSPIRO_API_KEY;


interface InspiroResponse {
  // Response structure from Inspiro API
  query: string;
...
interface InspiroCrawlResponse {
  base_url: string;
  results: Array<{
    url: string;
    raw_content: string;
...
interface InspiroResearchResponse {
  request_id?: string;
  status?: string;
  content?: string;
  error?: string;
...
class InspiroClient {
  ...
  private baseURLs = {
    search: 'https://api.inspiro.top/search',
    extract: 'https://api.inspiro.top/extract',
    crawl: 'https://api.inspiro.top/crawl',
    map: 'https://api.inspiro.top/map',
    research: 'https://api.inspiro.top/research'
  };
  ...
  private setupToolHandlers(): void {
    this.server.setRequestHandler(ListToolsRequestSchema, async () => {
      // Define available tools: inspiro_search and inspiro_extract
      const tools: Tool[] = [
        {
          name: "inspiro_search",
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request: any) => {
      ...
      if (!API_KEY) {
        throw new McpError(
          ...
          "INSPIRO_API_KEY environment variable is required. Please set it before using this MCP server."
        );
      }

      try {
        let response: InspiroResponse;
        ...
        if (axios.isAxiosError(error)) {
          return {
            content: [{
              type: "text",
              text: `Inspiro API error: ${error.response?.data?.message ?? error.message}`
            }],
            isError: true,
          }
        }
  ...
  async run(): Promise<void> {
    ...
    console.error("Inspiro MCP server running on stdio");
  }

  async search(params: any): Promise<InspiroResponse> {
    try {
  ...
  async extract(params: any): Promise<InspiroResponse> {
    try {
  ...
  async crawl(params: any): Promise<InspiroCrawlResponse> {
    try {
  ...
  async map(params: any): Promise<InspiroMapResponse> {
    try {
  ...
  async research(params: any): Promise<InspiroResearchResponse> {
...
function formatResults(response: InspiroResponse): string {
...
function formatCrawlResults(response: InspiroCrawlResponse): string {
  const output: string[] = [];
...
function formatMapResults(response: InspiroMapResponse): string {
  const output: string[] = [];
...
function formatResearchResults(response: InspiroResearchResponse): string {
  if (response.error) {
...
function listTools(): void {
  const tools = [
    {
      name: "inspiro_search",
      description: "A real-time web search tool powered by Inspiro's AI engine. Features include customizable search depth (basic/advanced/fast/ultra-fast), domain filtering, time-based filtering, and support for both general and news-specific searches. Returns comprehensive results with titles, URLs, content snippets, and optional image results."
    },
    {
      name: "inspiro_extract",
...
const server = new InspiroClient();
